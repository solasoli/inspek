...
use App\KodeRekeningExcelCollection;
use App\Config;
...
class KodeRekeningController extends Controller
{
    ...
    public function store_temp(Request $request)
    {
      ...
      if ($validator->fails()) {
        ...
        try {
          $store = Excel::import(new KodeRekeningExcelCollection, request()->file('file'));
          $last_data = DB::table('temp_kode_rekening')->latest('id')->first();
          return redirect('/upload_kode_rekening/verify_temp/'.$last_data->id);
        }
        catch (\Exception $e) {
