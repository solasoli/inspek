...
class LaporanLhpController extends Controller
{
    ...
    public function submit_review($id_kertas_kerja, Request $request){
        if($request->input('step_approve') == 'review') {
            AuditService::review($id_kertas_kerja, $request->input(), 'lhp');
            $request->session()->flash('success', "Data berhasil disimpan!");
            return redirect("/pemeriksaan/laporan_lhp/review/".$id_kertas_kerja);
        } else {
            
            $data = KertasKerja::findOrFail($id_kertas_kerja);
            AuditService::approve($id_kertas_kerja, 'lhp');
            $request->session()->flash('success', "Data berhasil disimpan!");
            return redirect("/pemeriksaan/laporan_lhp/review_list/".$data->surat_perintah->id);
        }
    }
