...
use App\Repository\Pegawai\Pangkat;
...
class PangkatService
{
    ...
    {
        $t = Pangkat::findOrFail($id);
        return self::proccess_data($t, $data);
    }

    public static function createOrUpdate($id, $data)
    {
        $t = Pangkat::findOrNew($id);
        return self::proccess_data($t, $data);
    }

    private static function proccess_data(Pangkat $eselon, $data)
    {
    ...
    {
        $t = Pangkat::findOrFail($id)->delete();
    }

    public static function get_data($is_deleted = 0)
    {
        return Pangkat::where("is_deleted", $is_deleted)->get();
    }
}
